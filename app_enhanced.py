...
page = st.sidebar.selectbox(
    "Navigate to:",
    ["🏠 Home", "📚 Study Materials", "🎯 ProSort Euler", "🔐 PWNHUB CTF", "📝 Blog", "📅 Events", "👥 About Us", "🧮 Math Tools"]
)
...
if page == "🏠 Home":
    ...
    st.markdown('<h3 class="section-header">🎨 Interactive Mathematics</h3>', unsafe_allow_html=True)
    
    col1, col2 = st.columns(2)
    with col1:
        st.subheader("📊 Fibonacci Sequence")
        n_fib = st.slider("Number of terms:", 5, 50, 20)
        fib_sequence = math_tools.fibonacci_sequence(n_fib)
        
        fig_fib = px.line(x=list(range(n_fib)), y=fib_sequence, 
                         title=f"First {n_fib} Fibonacci Numbers",
                         labels={'x': 'Index', 'y': 'Value'})
        st.plotly_chart(fig_fib, use_container_width=True)
    
    with col2:
        st.subheader("🔢 Prime Number Distribution")
        n_primes = st.slider("Number of primes:", 10, 100, 50)
        primes = math_tools.generate_primes(n_primes)
        
        fig_primes = px.scatter(x=list(range(1, n_primes + 1)), y=primes,
                              title=f"First {n_primes} Prime Numbers",
                              labels={'x': 'Position', 'y': 'Prime Number'})
        st.plotly_chart(fig_primes, use_container_width=True)

# Math Tools page
elif page == "🧮 Math Tools":
    st.markdown('<h2 class="section-header">🧮 Advanced Mathematical Tools</h2>', unsafe_allow_html=True)
    
    # Tool selection
    tool_type = st.selectbox(
        "Select Tool:",
        ["Matrix Operations", "Equation Solver", "Function Plotter", "Number Theory", "Linear Algebra"]
    )
    
    if tool_type == "Matrix Operations":
        st.subheader("🔢 Matrix Operations")
        
        # Matrix input
        matrix_size = st.selectbox("Matrix Size:", [2, 3, 4])
        
        st.markdown("Enter matrix elements:")
        matrix = []
        ...
        for i in range(matrix_size):
            ...
            for j in range(matrix_size):
                with cols[j]:
                    val = st.number_input(f"a_{i+1}{j+1}", value=1.0 if i == j else 0.0, key=f"m_{i}_{j}")
                    row.append(val)
            matrix.append(row)
        ...
        with col1:
            if st.button("Calculate Determinant"):
                det = math_tools.matrix_determinant(matrix)
                st.success(f"**Determinant:** {det:.4f}")
            
            if st.button("Find Rank"):
                rank = math_tools.matrix_rank(matrix)
                st.success(f"**Rank:** {rank}")
        
        with col2:
            if st.button("Find Eigenvalues"):
                eigenvalues, eigenvectors = math_tools.find_eigenvalues(matrix)
                st.success(f"**Eigenvalues:** {eigenvalues}")
                st.write("**Eigenvectors:**")
                st.write(eigenvectors)
        
        # Matrix visualization
        if st.button("Visualize Matrix"):
            fig = math_tools.matrix_visualization(matrix)
            st.plotly_chart(fig, use_container_width=True)
    
    elif tool_type == "Equation Solver":
        st.subheader("🔍 Equation Solver")
        
        eq_type = st.selectbox("Equation Type:", ["Linear", "Quadratic", "System of Linear Equations"])
        ...
        if eq_type == "Linear":
            ...
            if st.button("Solve"):
                if a != 0:
                    x = (c - b) / a
                    st.success(f"**Solution:** x = {x:.4f}")
                else:
                    st.error("Invalid equation: 'a' cannot be zero")
        
        elif eq_type == "Quadratic":
            st.markdown("**Solve: ax² + bx + c = 0**")
            col1, col2, col3 = st.columns(3)
            with col1:
                a = st.number_input("a", value=1.0, key="a_quad")
            with col2:
                b = st.number_input("b", value=-5.0, key="b_quad")
            with col3:
                c = st.number_input("c", value=6.0, key="c_quad")
            
            if st.button("Solve"):
                roots, message = math_tools.solve_quadratic(a, b, c)
                st.success(f"**Result:** {message}")
                if roots:
                    st.write("**Roots:**", roots)
    
    elif tool_type == "Function Plotter":
        st.subheader("📈 Function Plotter")
        
        st.markdown("**Plot a mathematical function**")
        st.markdown("Available functions: sin(x), cos(x), tan(x), exp(x), log(x), sqrt(x)")
        
        func_str = st.text_input("Enter function (e.g., sin(x), x**2 + 2*x + 1):", value="sin(x)")
        
        col1, col2 = st.columns(2)
        with col1:
            x_min = st.number_input("X Min:", value=-10.0, key="x_min")
        with col2:
            x_max = st.number_input("X Max:", value=10.0, key="x_max")
        
        if st.button("Plot Function"):
            fig = math_tools.plot_function(func_str, (x_min, x_max))
            st.plotly_chart(fig, use_container_width=True)
    
    elif tool_type == "Number Theory":
        st.subheader("🔢 Number Theory Tools")
        
        num_tool = st.selectbox("Select Tool:", ["Prime Checker", "Prime Factorization", "GCD & LCM", "Modular Inverse"])
        
        if num_tool == "Prime Checker":
            n = st.number_input("Enter a number:", value=17, min_value=1, step=1)
            if st.button("Check if Prime"):
                is_prime = math_tools.is_prime(int(n))
                if is_prime:
                    st.success(f"**{int(n)} is PRIME!**")
                else:
                    st.error(f"**{int(n)} is NOT PRIME**")
        
        elif num_tool == "Prime Factorization":
            n = st.number_input("Enter a number:", value=100, min_value=1, step=1)
            if st.button("Find Prime Factors"):
                factors = math_tools.prime_factorization(int(n))
                st.success(f"**Prime factors of {int(n)}:** {factors}")
        
        elif num_tool == "GCD & LCM":
            col1, col2 = st.columns(2)
            with col1:
                a = st.number_input("First number:", value=48, min_value=1, step=1)
            with col2:
                b = st.number_input("Second number:", value=18, min_value=1, step=1)
            
            if st.button("Calculate"):
                gcd_val = math_tools.gcd(int(a), int(b))
                lcm_val = math_tools.lcm(int(a), int(b))
                st.success(f"**GCD({a}, {b}) = {gcd_val}**")
                st.success(f"**LCM({a}, {b}) = {lcm_val}**")
...
# Study Materials page
    ...
    if selected_subject == "Linear Algebra":
        ...
        **Recommended Books:**
        ...
        primes = math_tools.generate_primes(n)
